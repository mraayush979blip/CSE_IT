    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end border-b border-slate-300 pb-1">
        <div className="flex space-x-2">
          <button
            onClick={() => activeTab !== 'students' && navigate('/admin/students', { replace: true })}
          ...
          <button
            onClick={() => activeTab !== 'faculty' && navigate('/admin/faculty', { replace: true })}
          ...
          <button
            onClick={() => activeTab !== 'monitor' && navigate('/admin/monitor', { replace: true })}
          ...
          <button
            onClick={() => activeTab !== 'reports' && navigate('/admin/reports', { replace: true })}
          ...
          <button
            onClick={() => activeTab !== 'system' && navigate('/admin/system', { replace: true })}
    ...
    <AdminStudentDetail
      ...
      onBack={() => {
        if (window.history.length > 1) {
          ...
          navigate(`/admin/students/${branchId}/${batchId}`, { replace: true });
        }
      }}
    />
  );
    ...
    <Card>
      ...
      <div className="flex items-center text-sm mb-6 text-slate-500 flex-wrap justify-between">
        <div className="flex items-center">
          <span className={`cursor-pointer hover:text-indigo-600 ${level === 'branches' ? 'font-bold text-indigo-600' : ''}`} onClick={() => level !== 'branches' && navigate('/admin/students', { replace: true })}>Classes</span>
          {selBranch && <><ChevronRight className="h-4 w-4 mx-2" /><span className={`cursor-pointer hover:text-indigo-600 ${level === 'batches' ? 'font-bold text-indigo-600' : ''}`} onClick={() => level !== 'batches' && navigate(`/admin/students/${branchId}`, { replace: true })}>{selBranch.name}</span></>}
